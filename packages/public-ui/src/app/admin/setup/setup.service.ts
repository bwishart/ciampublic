...
export class SetupService {
    ...
    public getAttributes() {
        this.http.get<any>(`${this.baseurl}/v2.0/schema-attributes`).subscribe({
            next: (res: any) => {
            ...
            error: (err: ISVError) => {
                ...
                this.notification.sendError(`${errorPrefix} ${err.message}`);
            },
        });
        this.http.get<any>(`${this.baseurl}/v1.0/attributes`).subscribe({
            next: (res: IAttribute[]) => {
                this.attributes = res;
    ...
    public createAttributes(attributeIds: string[]) {
        ...
        attributeIds.forEach((attrId, index) => {
            ...
            apiData.push(
                this.http
                    .post<any>(`${this.baseurl}/v1.0/attributes`, attrPayload)
        ...
        requiredAttributeIds.forEach(requiredAttrId => {
            if (
                !this.attributes?.find(
                    (attr: IAttribute) =>
                        requiredAttrId.toLowerCase() ===
                        attr.schemaAttribute.scimName.toLowerCase()
                )
            ) {
    ...
    testISVApplicationConfiguration() {
        ...
        this.http
            .get<any>(
                `${this.baseurl}/setup/getApplicationDetails/${this.configService?.tenantConfig?.ISV_APP_ID}`
            )
            .subscribe({
    ...
    uploadTheme(file, themeName) {
        ...
        const url = `${this.baseurl}/v1.0/themes`;
        let request = this.http.post<any>(url, formData);
        if (themeID) {
            request = this.http.put<any>(`${url}?id=${themeID}`, formData);
        }
    ...
    saveTenantConfig(payload: ITenantConfig) {
        return this.http.put<any>(`${this.baseurl}/configure-tenant`, payload);
    }
    ...
    saveISVapplication() {
        ...
        return this.http
            .put<any>(
                `${this.baseurl}/setup/updateApplicationDetails/${this.configService?.tenantConfig?.ISV_APP_ID}`,
                payload
            )
            .subscribe({
                next: (application: IApplicationDetails) => {
