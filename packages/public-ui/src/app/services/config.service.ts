...
export class ConfigService {
    ...
    initConfig(): any {
        this.http
            .get<any>(`${environment.baseUrl}${configPath}`)
            .subscribe(res => {
                this.config = res;
                this.configEvent.emit(res);
            });
    ...
    getFormattedConfig(options, res, compName, propName, parentStyles?) {
        ...
        return options.map(option => {
            ...
            if (foundOption) {
                const result = {
                    ...(parentStyles?.color && { color: parentStyles?.color }),
    ...
    getISVTemplate(path) {
        ...
        return this.http
            .get<any>(
                `${environment.baseUrl}/v1.0/themes?path=${this.tenantConfig.THEME_ID}${path}`,
                requestOptions
            )
    ...
    saveISVTemplate(path, templateJSON, saveISVLabels?) {
        ...
        return this.http
            .put<any>(
                `${environment.baseUrl}/v1.0/themes?id=${this.tenantConfig.THEME_ID}&path=${path}`,
                formData,
                requestOptions
            )
            .subscribe(() => {
