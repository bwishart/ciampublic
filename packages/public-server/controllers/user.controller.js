...
const findAccount = async (baseUrl, ssn, token) => {
    const options = {
        method: 'get',
        url: `${baseUrl}/v2.0/Users?filter=urn:ietf:params:scim:schemas:extension:ibm:2.0:User:customAttributes.accountId eq "${ssn}" and active eq "true"`,
        headers: {
...
module.exports.resetUsers = async (req, res, next) => {
    ...
    userJson.forEach(async (user) => {
        const id = await findAccount(baseUrl, user.ssn, token);
        if (id) {
            const isDeleted = userDelete(baseUrl, id, token);
            if (isDeleted) delete newUserJson[user.ssn];
        }
    });
...
module.exports.saveUserDetails = (req, res, next) => {
    ...
    const userJson = JSON.parse(fs.readFileSync(userJsonPath, 'utf-8'));
    if (!userExists(userData.ssn.toString(), userJson)) {
        ...
        const err = new ErrorClass('SSN already exists', 403);
        next(err);
    }
};
function userExists(ssn, userJson) {
    if (userJson.length) {
        const userFound = userJson.find((user) => user.ssn === ssn);
        return userFound;
    }
    return false;
}
