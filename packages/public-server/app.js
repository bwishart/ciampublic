...
const v2Router = require('./routes/v2.0.routes');
const v1Router = require('./routes/v1.0.routes');
const {
    getTenantInfo,
    authorize,
    checkTokens,
...
if (process.env.ENABLE_ADMIN === 'false') {
    app.use('/setup', (req, res) => {
        res.status(401).send("Don't have permission to access the endpoint");
    });
...
app.get('/admin-flag', (req, res) => {
    return res.status(200).send(process.env.ENABLE_ADMIN || 'false');
});
...
if (protocol === 'https') {
    https
        ...
        .listen(port, () => {
            console.log(protocol, 'server is runing at port ', port);
        });
} else {
    app.listen(port, () => {
        console.log(protocol, 'Server is *_* at port', port);
    });
}
        ...
        new OpenIDStrategy(
            {
                ...
                callbackURL: `${baseurl}${config.OIDC_REDIRECT_URI}`,
